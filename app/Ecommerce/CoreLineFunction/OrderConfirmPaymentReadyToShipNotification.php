...
use YellowProject\Ecommerce\LineTemplateMessage\PaymentConfirmation\LinePaymentConfirmation;
...
class OrderConfirmPaymentReadyToShipNotification extends Model
{
    public static function pushNotificationCustomer($order,$customer,$orderConfirmation,$orderConfirmationPaymentReadyToShip)
    {
        ...
        // $messages = self::setDataConfirmation($order,$customer,$orderConfirmation,$orderConfirmationPaymentReadyToShip,$lineUserProfile);
        // $message = collect($messages);

        $messages = self::setSendMessageToCustomer($order,$customer,$orderConfirmation,$orderConfirmationPaymentReadyToShip,$lineUserProfile);
        $message = collect($messages);
        LineFunction::pushMessage($lineUserProfile,$message);
        self::autoPushNotificationDT($order,$customer,$orderConfirmation,$orderConfirmationPaymentReadyToShip);
        self::autoPushNotificationAdmin($order,$customer,$orderConfirmation,$orderConfirmationPaymentReadyToShip);

        return 1;
    }

    public static function autoPushNotificationAdmin($order,$customer,$orderConfirmation,$orderConfirmationPaymentReadyToShip)
    {
        ...
        foreach ($adminLineUsers as $key => $adminLineUser) {
            ...
            $messages = self::setSendMessageToAdmin($order,$customer,$orderConfirmation,$orderConfirmationPaymentReadyToShip,$lineUserProfile);
            $message = collect($messages);
            LineFunction::pushMessage($lineUserProfile,$message);
        }
    ...
    public static function autoPushNotificationDT($order,$customer,$orderConfirmation,$orderConfirmationPaymentReadyToShip)
    {
        $customer = $order->customer;
        $dtID = $order->dt_id;
        ...
        foreach ($DTManagementRegisterDatas as $key => $DTManagementRegisterData) {
            ...
            $messages = self::setSendMessageToDT($order,$customer,$orderConfirmation,$orderConfirmationPaymentReadyToShip,$lineUserProfile);
            $message = collect($messages);
            LineFunction::pushMessage($lineUserProfile,$message);
        }
    ...
    public static function setSendMessageToAdmin($order,$customer,$orderConfirmation,$orderConfirmationPaymentReadyToShip,$lineUserProfile)
    {
        $dtID = $customer->dt_id;
        $DTManagement = DTManagement::find($dtID);
        $orderConfirmation = ReadyToShip::first();
        $lineOrderConfirmation = LinePaymentConfirmation::where('id',$orderConfirmation->line_to_admin_template_id)->first();
        $setData = self::setDataConfirmation($order,$customer,$orderConfirmation,$orderConfirmationPaymentReadyToShip,$lineUserProfile,$lineOrderConfirmation,$DTManagement);

        return $setData;
    }

    public static function setSendMessageToDT($order,$customer,$orderConfirmation,$orderConfirmationPaymentReadyToShip,$lineUserProfile)
    {
        $dtID = $customer->dt_id;
        $DTManagement = DTManagement::find($dtID);
        $orderConfirmation = ReadyToShip::first();
        $lineOrderConfirmation = LinePaymentConfirmation::where('id',$orderConfirmation->line_to_dt_template_id)->first();
        $setData = self::setDataConfirmation($order,$customer,$orderConfirmation,$orderConfirmationPaymentReadyToShip,$lineUserProfile,$lineOrderConfirmation,$DTManagement);

        return $setData;
    }

    public static function setSendMessageToCustomer($order,$customer,$orderConfirmation,$orderConfirmationPaymentReadyToShip,$lineUserProfile)
    {
        // $lineConfirmation = $order->orderConfirmationPaymentReadyToShip;
        // $lineOrderConfirmation = LinePaymentConfirmation::where('id',$lineConfirmation->line_to_customer_template_id)->first();
        // $setData = self::setDataConfirmation($order,$customer,$orderConfirmation,$orderConfirmationPaymentReadyToShip,$lineUserProfile,$lineOrderConfirmation);

        // return $setData;
        $dtID = $customer->dt_id;
        $DTManagement = DTManagement::find($dtID);
        $orderConfirmation = ReadyToShip::first();
        $lineOrderConfirmation = LinePaymentConfirmation::where('id',$orderConfirmation->line_to_customer_template_id)->first();
        $setData = self::setDataConfirmation($order,$customer,$orderConfirmation,$orderConfirmationPaymentReadyToShip,$lineUserProfile,$lineOrderConfirmation,$DTManagement);

        return $setData;
    }

    public static function setDataConfirmation($order,$customer,$orderConfirmation,$orderConfirmationPaymentReadyToShip,$lineUserProfile,$lineOrderConfirmation,$DTManagement)
    {
        // $lineConfirmation = $order->orderConfirmationPaymentReadyToShip;
        // $lineOrderConfirmation = LinePaymentConfirmation::where('id',$lineConfirmation->line_to_customer_template_id)->first();
            ...
            if (sizeof($items) > 0) {
                foreach ($items as $key => $item) {
                    if ($item->messageType->type == 'text') {
                        ...
                        $messages[$key]  = [
                            "type" =>"text",
                            "text" => self::sentPayload($item->message->message,$lineUserProfile,$order,$customer,$DTManagement)
                        ];
    ...
    public static function sentPayload($payload,$lineUserProfile,$order,$customer,$DTManagement)
    {
        $string='';
        $subscriberID = '';
        $keyword = '';
