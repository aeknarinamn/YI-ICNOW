...
use YellowProject\Ecommerce\LineTemplateMessage\CustomerPaymentOverdue72\CustomerPaymentOverdue72;
use YellowProject\Ecommerce\LineTemplateMessage\LineTemplateMessageAuto\CustomerPaymentOverdue72 as LineCustomerPaymentOverdue72;
...
class OrderOverDuePayment72 extends Model
{
    public static function sentMessageCustomerPaymentOverdue72($customer,$order)
    {
    ...
    public static function autoPushNotificationDT($customer,$order)
    {
        ...
        foreach ($DTManagementRegisterDatas as $key => $DTManagementRegisterData) {
            ...
            $messages = self::setSendMessageToDT($customer,$order,$lineUserProfile);
            $message = collect($messages);
            self::pushMessage($lineUserProfile,$message);
        }
    ...
    public static function setSendMessageToAdmin($customer,$order,$lineUserProfile)
    {
        $dtID = $customer->dt_id;
        $DTManagement = DTManagement::find($dtID);
        $orderConfirmation = LineCustomerPaymentOverdue72::first();
        $lineOrderConfirmation = CustomerPaymentOverdue72::where('id',$orderConfirmation->line_to_admin_template_id)->first();
        $setData = self::setDataConfirmation($customer,$order,$lineOrderConfirmation,$DTManagement);

        return $setData;
    }

    public static function setSendMessageToDT($customer,$order,$lineUserProfile)
    {
        $dtID = $customer->dt_id;
        $DTManagement = DTManagement::find($dtID);
        $orderConfirmation = LineCustomerPaymentOverdue72::first();
        $lineOrderConfirmation = CustomerPaymentOverdue72::where('id',$orderConfirmation->line_to_dt_template_id)->first();
        $setData = self::setDataConfirmation($customer,$order,$lineOrderConfirmation,$DTManagement);
    ...
    public static function setSendMessageToCustomer($customer,$order)
    {
        ...
        $DTManagement = DTManagement::find($dtID);
        $orderConfirmation = LineCustomerPaymentOverdue72::first();
        $lineOrderConfirmation = CustomerPaymentOverdue72::where('id',$orderConfirmation->line_to_customer_template_id)->first();
        $setData = self::setDataConfirmation($customer,$order,$lineOrderConfirmation,$DTManagement);

        return $setData;
    }

    public static function setDataConfirmation($customer,$order,$lineOrderConfirmation,$DTManagement)
    {
        $messages = array();
    ...
    public static function sentPayload($payload,$order,$customer,$DTManagement)
    {
        $string='';
        $subscriberID = '';
        $keyword = '';
