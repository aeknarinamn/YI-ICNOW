...
use YellowProject\Ecommerce\LineTemplateMessage\CustomerConfirmPayment\LineCustomerConfirmPayment;
...
class OrderConfirmPaymentNotification extends Model
{
	public static function pushNotificationCustomer($order,$orderPayment)
    {
    	...
    	$messages = self::setSendMessageToCustomer($order,$customer,$orderPayment,$lineUserProfile);
        $message = collect($messages);
        LineFunction::pushMessage($lineUserProfile,$message);
        self::autoPushNotificationDT($order,$customer,$orderPayment);
        self::autoPushNotificationAdmin($order,$customer,$orderPayment);

        return 1;
    }

    public static function autoPushNotificationAdmin($order,$customer,$orderPayment)
    {
        $customer = $order->customer;
        // $dtID = $customer->dt_id;
        ...
        foreach ($adminLineUsers as $key => $adminLineUser) {
            ...
            $messages = self::setSendMessageToAdmin($order,$customer,$orderPayment,$lineUserProfile);
            $message = collect($messages);
            LineFunction::pushMessage($lineUserProfile,$message);
        }
    ...
    public static function autoPushNotificationDT($order,$customer,$orderPayment)
    {
        $customer = $order->customer;
        $dtID = $order->dt_id;
        ...
        foreach ($DTManagementRegisterDatas as $key => $DTManagementRegisterData) {
            ...
            $messages = self::setSendMessageToDT($order,$customer,$orderPayment,$lineUserProfile);
            $message = collect($messages);
            LineFunction::pushMessage($lineUserProfile,$message);
        }
    ...
    public static function setSendMessageToAdmin($order,$customer,$orderPayment,$lineUserProfile)
    {
        $dtID = $customer->dt_id;
        $DTManagement = DTManagement::find($dtID);
        if($orderPayment->payment_type == 'COD'){
            $orderConfirmation = CustomerConfirmPaymentCOD::first();
        }else{
            $orderConfirmation = CustomerConfirmPaymentBankTransfer::first();
        }
        $lineOrderConfirmation = LineCustomerConfirmPayment::where('id',$orderConfirmation->line_to_admin_template_id)->first();
        $setData = self::setDataConfirmation($order,$customer,$orderPayment,$lineUserProfile,$lineOrderConfirmation,$DTManagement);

        return $setData;
    }

    public static function setSendMessageToDT($order,$customer,$orderPayment,$lineUserProfile)
    {
        $dtID = $customer->dt_id;
        $DTManagement = DTManagement::find($dtID);
        if($orderPayment->payment_type == 'COD'){
        ...
        $lineOrderConfirmation = LineCustomerConfirmPayment::where('id',$orderConfirmation->line_to_dt_template_id)->first();
        $setData = self::setDataConfirmation($order,$customer,$orderPayment,$lineUserProfile,$lineOrderConfirmation,$DTManagement);

        return $setData;
    }

    public static function setSendMessageToCustomer($order,$customer,$orderPayment,$lineUserProfile)
    {
        $dtID = $customer->dt_id;
        $DTManagement = DTManagement::find($dtID);
        if($orderPayment->payment_type == 'COD'){
        ...
        $lineOrderConfirmation = LineCustomerConfirmPayment::where('id',$orderConfirmation->line_to_customer_template_id)->first();
        $setData = self::setDataConfirmation($order,$customer,$orderPayment,$lineUserProfile,$lineOrderConfirmation,$DTManagement);

        return $setData;
    }

    public static function setDataConfirmation($order,$customer,$orderPayment,$lineUserProfile,$lineOrderConfirmation,$DTManagement)
    {
            ...
            if (sizeof($items) > 0) {
                foreach ($items as $key => $item) {
                    if ($item->messageType->type == 'text') {
                        ...
                        $messages[$key]  = [
                            "type" =>"text",
                            "text" => self::sentPayload($item->message->message,$order,$customer,$orderPayment,$DTManagement)
                        ];
    ...
    public static function sentPayload($payload,$order,$customer,$orderPayment,$DTManagement)
    {
        $string='';
        $subscriberID = '';
        $keyword = '';
