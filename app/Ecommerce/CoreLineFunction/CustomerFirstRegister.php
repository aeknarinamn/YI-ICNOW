...
use YellowProject\Ecommerce\LineTemplateMessage\FirstRegister\FirstRegister;
use YellowProject\Ecommerce\LineTemplateMessage\LineTemplateMessageAuto\FirstRegister as LineFirstRegister;
...
class CustomerFirstRegister extends Model
{
    public static function sentMessageCustomerFirstRegister($customer)
    {
        $lineUserProfile = $customer->lineUserProfile;

        $orderConfirmation = FirstRegister::first();
        if($orderConfirmation && $customer->dt_id != -1){
            $messages = self::setSendMessageToCustomer($customer);
            $message = collect($messages);
            self::pushMessage($lineUserProfile,$message);
            self::autoPushNotificationDT($customer);
            self::autoPushNotificationAdmin($customer);
        }

        return 1;
    }
    
    public static function autoPushNotificationAdmin($customer)
    {
        ...
        foreach ($adminLineUsers as $key => $adminLineUser) {
            ...
            $messages = self::setSendMessageToAdmin($customer);
            $message = collect($messages);
            self::pushMessage($lineUserProfile,$message);
        }
    ...
    public static function autoPushNotificationDT($customer)
    {
        $dtID = $customer->dt_id;
        ...
        foreach ($DTManagementRegisterDatas as $key => $DTManagementRegisterData) {
            ...
            $messages = self::setSendMessageToDT($customer);
            $message = collect($messages);
            self::pushMessage($lineUserProfile,$message);
        }
    ...
    public static function setSendMessageToAdmin($customer)
    {
        $dtID = $customer->dt_id;
        $DTManagement = DTManagement::find($dtID);
        $orderConfirmation = LineFirstRegister::first();
        $lineOrderConfirmation = FirstRegister::where('id',$orderConfirmation->line_to_admin_template_id)->first();
        $setData = self::setDataConfirmation($customer,$lineOrderConfirmation,$DTManagement);

        return $setData;
    }

    public static function setSendMessageToDT($customer)
    {
        $dtID = $customer->dt_id;
        $DTManagement = DTManagement::find($dtID);
        $orderConfirmation = LineFirstRegister::first();
        $lineOrderConfirmation = FirstRegister::where('id',$orderConfirmation->line_to_dt_template_id)->first();
        $setData = self::setDataConfirmation($customer,$lineOrderConfirmation,$DTManagement);

        return $setData;
    }

    public static function setSendMessageToCustomer($customer)
    {
        $dtID = $customer->dt_id;
        $DTManagement = DTManagement::find($dtID);
        $orderConfirmation = LineFirstRegister::first();
        $lineOrderConfirmation = FirstRegister::where('id',$orderConfirmation->line_to_customer_template_id)->first();
        $setData = self::setDataConfirmation($customer,$lineOrderConfirmation,$DTManagement);

        return $setData;
    }

    public static function setDataConfirmation($customer,$lineOrderConfirmation,$DTManagement)
    {
        $messages = array();
            ...
            if (sizeof($items) > 0) {
                foreach ($items as $key => $item) {
                    if ($item->messageType->type == 'text') {
                        ...
                        $messages[$key]  = [
                            "type" =>"text",
                            "text" => self::sentPayload($item->message->message,$customer,$DTManagement)
                        ];
    ...
    public static function sentPayload($payload,$customer,$DTManagement)
    {
        $string='';
        $subscriberID = '';
        $keyword = '';
