...
<body>
    ...
    <div id="alertCheckAreaModal" class="modal">
        <div class="modal-background" id="backgroundModal"></div>
        <div class="modal-content">
            <div class="modal-title">กรุณารอสักครู่</div>
            <div class="modal-detail">ระบบกำลังตรวจสอบพื้นที่การให้บริการกรุณารอสักครู่</div>
        </div>
    </div>

    <div id="alertAreaNoService" class="modal">
        <div class="modal-background" id="backgroundModal"></div>
        <div class="modal-content">
            <div class="modal-title">ขออภัย</div>
            <div class="modal-detail">พื้นที่ของท่านไม่อยุ่ในพื้นที่ให้บริการ</div>
            <div class="modal-button">
                <a href="javascript:closeModal()" class="image-btn">
                    <img src="/icnow/resources/images/btn-cancel.png" alt="Button">
                </a>
    ...
    <input type="hidden" id="isAddress" value="{{$isAddress}}">
    <input type="hidden" id="province" value="{{$cacheData->value}}">
    ...
    <script>
        ...
        function initMap() {
          ...
          function geocodeLatLng(lat,lng) {
            ...
            geocoder.geocode({'location': latlng}, function(results, status) {
              if (status === 'OK') {
                ...
                var contentString = '<a href="#" onclick="submitForm(); return false;">'+results[1].formatted_address+'</a>';
          ...
          function closeModal(){
            $('#alertModal').hide();
            $('#alertCheckAreaModal').hide();
            $('#alertAreaNoService').hide();
          }
          ...
          function submitForm(){
            var alertCheckAreaModal = document.getElementById('alertCheckAreaModal');
            alertCheckAreaModal.style.display = "block";
            var lat = $('#lat').val();
            var long = $('#long').val();
            $isCheck = 1;
            if(lat == "" || long == ""){
            ...
            if($isCheck == 1){
                $.ajax({
                    method: "POST",
                    url: "/api/icnow-address-check-area-service",
                    data: { 
                        lat: lat,
                        long: long,
                    }
                ...
                .done(function( result ) {
                    if(result.is_area == 1){
                      ...
                      alertCheckAreaModal.style.display = "none";
                      var modal = document.getElementById('alertAreaNoService');
